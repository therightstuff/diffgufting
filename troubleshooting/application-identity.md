...
**Result:** `app.setName('diffgufting')` runs only after `app.whenReady()`. Electron documents that `setName` changes its internal name but not the name used by the operating system. Electron Packager instead derives the application bundle name from `productName` or `name` in `package.json`.
