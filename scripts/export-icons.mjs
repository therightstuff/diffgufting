...
try {
  ...
  for (const size of [16, 32, 48, 64, 128, 256, 512, 1024]) {
    const png = await sharp('assets/branding/diffgusting-icon.png').resize(size, size, { kernel: 'nearest' }).png().toBuffer();
  ...
  await writeFile(`${destination}/diffgusting.ico`, Buffer.concat([header, ...sizes.map(size => images.get(size))]));
  const chunks = [];
  ...
  await writeFile(`${destination}/diffgusting.icns`, Buffer.concat([icns, ...chunks]));
