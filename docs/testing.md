...
Current focused suites cover document transitions and review, CLI/source parsing, filesystem comparison and guarded saving, Git layer snapshots, and atomic-replacement watching. The desktop suite covers editing, disjoint contention review, layout switching, undo, and theme selection, and writes a screenshot under ignored `test-results/`. Run `npm run build` before desktop tests. Teardown exits the owned test application directly so disposable unsaved buffers do not block cleanup.
