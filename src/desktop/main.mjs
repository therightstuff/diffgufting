...
let window; let session; let pendingClose = false; let dirty = false; let preferences = settings(); let initial; let initialComparison;
...
async function launch() {
  ...
  window = new BrowserWindow({ width: 1440, height: 960, minWidth: 900, minHeight: 600, title: 'Diffgusting', show: false, backgroundColor: '#171a21', icon: path.join(root, 'assets/branding/diffgusting-icon.png'), webPreferences: { preload: path.join(root, 'src/desktop/preload.cjs'), contextIsolation: true, nodeIntegration: false, sandbox: true } });
  ...
  ipc('bootstrap', async () => ({ preferences, comparison: initialComparison ?? null }));
  ...
  ipc('choose', async directory => {
    const chosen = await dialog.showOpenDialog(window, { properties: [directory ? 'openDirectory' : 'openFile'] });
